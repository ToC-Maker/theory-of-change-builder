...
// In-memory backend modelling the two rows `applyDeltaCommit` touches.
// Identical algebra to `delta-commit-concurrency.test.ts` (intentionally —
// the helper under test calls the same `applyDeltaCommit` import).
...
function makeBackend(initial: { message: LoggingMessageRow | null; user_usage: UserApiUsageRow }): {
  sql: NeonQueryFunction<false, false>;
  state: { message: LoggingMessageRow | null; user_usage: UserApiUsageRow };
  diagnosticInserts: { error_name: string; metadata: Record<string, unknown> }[];
} {
  const state = {
    message: initial.message ? { ...initial.message } : null,
    user_usage: { ...initial.user_usage },
  };
  const diagnosticInserts: { error_name: string; metadata: Record<string, unknown> }[] = [];
  ...
  // The sql tag distinguishes between the `applyDeltaCommit` CTE and the
  // `writeDiagnostic` INSERT by sniffing the joined template string.
  // applyDeltaCommit starts with `WITH locked AS (SELECT ...)` whereas
  // writeDiagnostic starts with `INSERT INTO logging_errors`.
  ...
  const sql: any = (strings: TemplateStringsArray, ...values: unknown[]) => {
    ...
    // interpolations repeat these but the values are identical.
    ...
    return withRowLock(messageId, async () => {
      await Promise.resolve(); // microtask yield for concurrency realism
  ...
  return { sql: sql as NeonQueryFunction<false, false>, state, diagnosticInserts };
}
