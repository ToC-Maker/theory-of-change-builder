...
// Identical algebra to `per-update-write-e2e.test.ts` (intentionally —
// `fireAbortCommit` calls the same `applyDeltaCommit` import).
...
function makeBackend(initial: { message: LoggingMessageRow | null; user_usage: UserApiUsageRow }): {
  ...
  const sql: any = (strings: TemplateStringsArray, ...values: unknown[]) => {
    ...
    // [2]=projected (string), [3]=newCost (string).
