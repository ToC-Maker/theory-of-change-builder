...
// In-memory representation of the two rows the helper touches.
...
/**
 * In-memory backend modelling the CTE the helper issues. The backend
 * exposes an SQL-tagged-template that:
 *
 *   - Looks up the locking row by (message_id, user_id, reconciled_at IS NULL).
 *     If absent, returns the empty-CTE shape (`applied=false`, both
 *     fields null), matching the real CTE's behaviour.
 *   - Otherwise computes baseline / delta / new_settled per the CTE's
 *     algebra and applies them transactionally. The `mutex` ensures
 *     two concurrent calls against the same message_id observe each
 *     other's writes (mirroring `FOR UPDATE` serialisation).
 *
 * The mutex is the simulation's stand-in for Postgres row-level locking.
 * In a real database, the `SELECT ... FOR UPDATE` blocks the second
 * writer until the first writer's transaction (here, the single
 * statement) commits; the second writer then re-reads the now-updated
 * row and computes its own baseline against the new value. The mutex
 * gives the same guarantee at the JS level: only one critical section
 * per message_id runs at a time.
 */
function makeBackend(initial: { message: LoggingMessageRow; user_usage: UserApiUsageRow }): {
  ...
  const sql: any = (_strings: TemplateStringsArray, ...values: unknown[]) => {
    // Per the helper's parameter order: messageId, userId, projStr, newStr
    // appear as the first four interpolations in the CTE. Subsequent
    // interpolations repeat them in the UPDATE clauses but the values
    ...
    return withRowLock(messageId, async () => {
      // Yield a microtask so concurrent callers race for the lock as they
      // would in a real network round-trip. Without this, a synchronous
      // critical section would always observe initial state and the
      // serialisation invariant wouldn't be exercised.
      await Promise.resolve();
      const row = state.message;
      const matches =
        row.message_id === messageId && row.user_id === userId && row.reconciled_at === null;
      if (!matches) {
        // CTE-empty shape: no rows matched the WHERE, so msg_upd RETURNING
        // produced zero rows and EXISTS evaluates to false.
  ...
  return { sql: sql as NeonQueryFunction<false, false>, state };
}
