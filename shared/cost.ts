 ...
 *
 * Rates come from `./pricing` (single source of truth). This module
 ...
 * as Number (IEEE 754).
 *
 * Lives in `shared/` so the React client and the Cloudflare Worker compute
 * prices identically (BigInt symmetry on the wire). The worker file at
 * `worker/_shared/cost.ts` is a thin re-export shim.
 *
 ...
 *
 * `total_tokens` is kept on the type for wire-shape parity with Anthropic's
 * SSE payload even though no consumer currently reads it; downstream code
 * that wants the sum computes it locally from the components.
 */
