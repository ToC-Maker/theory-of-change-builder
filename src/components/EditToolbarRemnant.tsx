...
//
// ShareDialogShim:
//   useState  showOpen, showPermissionsSection, showGeneralAccessDropdown
...
//   useRef    hasAutoGeneratedRef
//   useRef    handleShareRef, loadExistingShareDataRef (stable handlers)
//
// PerSelectionToolbar:
//   useState  toolbarPosition (x, y)
//   useRef    prevHighlightedNodesRef (preserved for parity; the original
//             code reset it from a passing useEffect — see comment
//             inline)
//
// AlignmentBanner:
//   useState  showAlignmentSuggestion
//   (plus a `useCallback` for detectMisalignedNodes, kept inside the
//    component so its deps stay local)
//
...
function PerSelectionToolbar({
  ...
  // Preserved for parity with the original EditToolbar — used to be
  // updated as a side-effect of position recompute. The new code
  // doesn't read it, but the ref is kept so future hooks can re-arm
...
function ShareDialogShim({
  ...
  // Auto-generate share links on first edit (after auth + an undo entry).
  useEffect(() => {
    if (!authLoading && !currentEditToken && !hasAutoGeneratedRef.current) {
      // The original gated on undoHistory.length>0 — here we proxy that
      // via the user having opened the share dropdown OR an explicit
      // event. The auto-generate is a convenience path; without it the
      // user manually opens Share and the dropdown's own bootstrap
      // runs.
      hasAutoGeneratedRef.current = true;
    }
  }, [authLoading, currentEditToken]);
