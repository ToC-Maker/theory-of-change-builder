...
export function EditToolbar({
  ...
  const [permissions, setPermissions] = useState<Array<{ user_id: string; user_email: string; permission_level: string; status?: string; granted_at: string }>>([])
  const [loadingPermissions, setLoadingPermissions] = useState(false)
  const [permissionError, setPermissionError] = useState<string | null>(null)
  const [linkSharingLevel, setLinkSharingLevel] = useState<'restricted' | 'viewer' | 'editor'>('restricted')
  ...
  }, [showRecentDropdown, isAuthenticated, user?.sub])
  ...
  }, [showShareDropdown, shareData?.chartId, isAuthenticated])
  ...
  useEffect(() => {
    ...
    if (!authLoading && !currentEditToken && !hasAutoGeneratedRef.current && undoHistory.length > 0) {
      ...
      const timer = setTimeout(() => {
        ...
        console.log('Auto-generating share links. Authenticated:', isAuthenticated, 'User:', user?.sub)
        handleShare()
      }, 100) // 100ms delay to ensure token is set

      return () => clearTimeout(timer)
    }
  }, [currentEditToken, undoHistory.length, authLoading, isAuthenticated, user])
                    ...
                    {loadingRecentCharts ? (
                      ...
                      <div className="border-t border-gray-200 pt-2">
                        <div className="px-3 py-1 text-xs font-medium text-gray-500 uppercase tracking-wider flex items-center justify-between">
                          ...
                          {!isAuthenticated && (
                            <span className="text-xs font-normal text-gray-400 normal-case">(Stored locally)</span>
                          )}
                        </div>
                        <div className="space-y-1 mt-1">
                          {recentCharts.map((chart, index) => {
                            const isCurrentChart = chart.editUrl === `${window.location.origin}/edit/${currentEditToken}`
                            const isOwner = chart.permissionLevel === 'owner'
                            return (
                              <div
                              ...
                              >
                                ...
                                >
                                  ...
                                  <div className="text-xs text-gray-500 mt-0.5 flex items-center gap-2">
                                    <span>
                                      {new Date(chart.updatedAt).toLocaleDateString()} at {new Date(chart.updatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                    </span>
                                    {isCurrentChart && <span className="text-blue-600">(Current)</span>}
                                    {chart.permissionLevel === 'edit' && (
                                      <span className="px-1.5 py-0.5 bg-purple-100 text-purple-700 rounded text-xs">Shared</span>
                                    )}
                                  </div>
                                </a>
                                {/* Delete button for owners or for all anonymous charts */}
                                {((isOwner && isAuthenticated) || (!isAuthenticated)) && chart.chartId && (
                                  <button
                                    onClick={(e) => {
                                      e.preventDefault()
                                      e.stopPropagation()
                                      handleDeleteChart(chart.chartId, chart.title)
                                    }}
                ...
                <div>
                  ...
                  {shareData && !shareLoading && (
                    <div className="space-y-4">
                      ...
                      {isAuthenticated && user?.sub && permissions.some(p => p.user_id === user.sub && p.permission_level === 'owner') && (
                        <div className="pt-3 border-t border-gray-200">
                          ...
                          >
                            ...
                            <ChevronDownIcon className={`w-4 h-4 transition-transform ${showPermissionsSection ? 'rotate-180' : ''}`} />
                          </button>
                            ...
                            <div className="mt-3 space-y-4">
                              ...
                              <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
                                ...
                                <p className="text-xs text-blue-700">
                                  Copy the <strong>Edit Link</strong> above and send it to collaborators. When they open it while logged in, they'll request access and you can approve them below.
                                </p>
                              </div>
                              ...
                              <div>
                                {loadingPermissions ? (
                                  ...
                                  <>
                                    ...
                                    {permissions.filter(p => p.status === 'pending').length > 0 && (
                                      <div className="mb-4">
                                        ...
                                        <div className="space-y-2">
                                          {permissions.filter(p => p.status === 'pending').map((perm) => {
                                    ...
                                    {/* Approved users */}
                                    {permissions.filter(p => p.status === 'approved' || !p.status).length > 0 && (
                                      <div>
                                        <label className="block text-sm font-medium text-gray-900 mb-2">
                                          People with access
                                        </label>
                                        <div className="space-y-2">
                                          {permissions.filter(p => p.status === 'approved' || !p.status).map((perm) => {
                                              ...
                                              >
                                                <div className="flex items-center gap-3 flex-1 min-w-0">
                                                  ...
                                                  <div className="flex-1 min-w-0">
                                                    <div className="text-sm font-medium text-gray-900 truncate">
                                                      {displayName}
                                                      {isOwner && user?.email === perm.user_email && (
                                                  ...
                                                  {isOwner ? (
                                                    ...
                                                    <>
                                                      <select
                                                        ...
                                                        onChange={(e) => handleUpdatePermission(perm.user_id, e.target.value as 'owner' | 'edit')}
                                    ...
                                    <div className="flex items-center gap-3">
                                      ...
                                      <div className="text-left">
                                        <div className="text-sm font-medium text-gray-900">
                                          {linkSharingLevel === 'restricted' ? 'Restricted' : 'Anyone with the link'}
                                        </div>
                                        <div className="text-xs text-gray-500">
                                          {linkSharingLevel === 'restricted' && 'Only people with explicit permission can access'}
                                          {linkSharingLevel === 'editor' && 'Anyone with the edit link can edit'}
                                          {linkSharingLevel === 'viewer' && 'Anyone with the link can view'}
                                        </div>
                                      </div>
                                    </div>
                                    <ChevronDownIcon className={`w-4 h-4 text-gray-500 flex-shrink-0 transition-transform ${showGeneralAccessDropdown ? 'rotate-180' : ''}`} />
                                  </button>
                                    ...
                                    <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50">
                                      ...
                                      >
                                        ...
                                        <div className="flex-1 min-w-0">
                                          ...
                                          <div className="text-xs text-gray-500">Only people with explicit permission can access</div>
                                        </div>
                                      </button>
                                      <button
                                        onClick={() => {
                                      ...
                                      >
                                        ...
                                        <div className="flex-1 min-w-0">
                                          ...
                                          <div className="text-xs text-gray-500">Anyone with the edit link can edit</div>
                                        </div>
                                      </button>
                                    </div>
                                  )}
