...
function ToCViewerOnly() {
  ...
  const loadFromLocalStorage = useCallback(
    (currentFilename?: string): ToCData | null => {
      try {
    ...
    return (
      <div className="min-h-screen w-full bg-white flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8 text-center">
          {/* Icon */}
          <div
            className={`mx-auto flex items-center justify-center h-16 w-16 rounded-full mb-4 ${isPendingError ? 'bg-yellow-100' : 'bg-red-100'}`}
          >
            {isPendingError ? (
              <svg
...
function ToCViewer() {
  ...
  const handleChartCreated = useCallback(
    async (editToken: string, chartId: string) => {
  ...
  const saveToLocalStorage = useCallback(
    (dataToSave: ToCData, currentFilename?: string) => {
      try {
        const storageKey = `toc_graph_${currentFilename || filename || 'default'}`;
    ...
    return () => {
      ...
      if (saveTimeoutRef.current) {
        ...
        if (pendingChangesRef.current && currentEditTokenRef.current) {
          ChartService.updateChart(currentEditTokenRef.current, pendingChangesRef.current).catch(
            console.error,
          );
        }
      }
